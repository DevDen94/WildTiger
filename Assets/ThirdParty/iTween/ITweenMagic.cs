...
public class ITweenMagic : MonoBehaviour
{
    ...
    void OnEnable()
    {
	   ...
	   // if (!ITweenSpace.Equals(MovementType.WorldSpace) && !ITweenSpace.Equals(MovementType.UISpace)) return;
	    //Movement
	    if (Movement)
	    {		
	...
	public void PlayRotation()
	{
			...
			iTween.ValueTo(this.gameObject, iTween.Hash("delay", delayRotation, "from",
				GetComponent<Transform>().localEulerAngles, "to", targetRotation,
				"time", timeRotation, "looptype", LoopTypeRotation.ToString(), "onupdate", "RotateObject", "oncomplete",
				"OnRotationTweenCompleted", "easetype", EaseTypeRotation.ToString()));
	}
