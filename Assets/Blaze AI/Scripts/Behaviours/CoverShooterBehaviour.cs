...
namespace BlazeAISpace
{
    ...
    public class CoverShooterBehaviour : MonoBehaviour
    {
        ...
        List<RaycastHit> canSeeTargetList = new List<RaycastHit>();
        RaycastHit[] strafingHitArr = new RaycastHit[10];
        List<RaycastHit> strafingHitList = new List<RaycastHit>();
        Collider[] callOthersHitArr = new Collider[20];
        RaycastHit[] checkTargetSeenHitArr = new RaycastHit[12];
        List<RaycastHit> checkTargetSeenList = new List<RaycastHit>();
        ...
        public virtual void Attack(bool movingBackwards = false)
        {
            isMoving = false;
        ...
        bool CanSeeTarget(bool onAttacking = false)
        {
            ...
            for (int i=0; i<hits; i++) 
            {
                ...
                canSeeTargetList.Add(canSeeTargetHitArr[i]);
            }

            if (canSeeTargetList.Count <= 0) {
                return false;
            }


            // sort the list by distance
            canSeeTargetList.Sort((x, y) => { return (x.distance).CompareTo((y.distance)); });


            // check not hitting the last cover of AI
            if (gtcBehaviour.lastCover != null) 
            {
                if (gtcBehaviour.lastCover.IsChildOf(canSeeTargetList[0].transform) || canSeeTargetList[0].transform.IsChildOf(gtcBehaviour.lastCover)) {
                    return false;
                }
            }
            ...
            if (onAttacking) 
            {
                if ((!blaze.enemyToAttack.transform.IsChildOf(canSeeTargetList[0].transform) && !canSeeTargetList[0].transform.IsChildOf(blaze.enemyToAttack.transform)) && (layersCheckOnAttacking & (1 << canSeeTargetList[0].transform.gameObject.layer)) != 0) {
            ...
            hitPoint = canSeeTargetList[0].point;
            ...
            if (shouldAttackCover) 
            {
                ...
                if (enemyCover) {
                    if (enemyCover.IsChildOf(canSeeTargetList[0].transform) || canSeeTargetList[0].transform.IsChildOf(enemyCover)) {
                        return true;
                    }
        ...
        {   
            ...
            for (int i=0; i<hits; i++) 
            {
                ...
                strafingHitList.Add(strafingHitArr[i]);
            }

            if (strafingHitList.Count > 0) {
                ChangeStrafeDirection();
                return;
            }
        ...
        public bool CheckIfTargetSeenFromPoint(Vector3 point, bool useNewCenterPos=false, float visionPosH = -1)
        {
            ...
            System.Array.Clear(checkTargetSeenHitArr, 0, checkTargetSeenHitArr.Length);
            checkTargetSeenList.Clear();

            checkTargetSeenHitArr = Physics.SphereCastAll(point + finalCenterPos, 0.05f, dir, Vector3.Distance(point, blaze.enemyColPoint) + 5, layers);
            int max = checkTargetSeenHitArr.Length;
            ...
            for (int i=0; i<max; i++) 
            {
                ...
                checkTargetSeenList.Add(checkTargetSeenHitArr[i]);
            }

            if (checkTargetSeenList.Count <= 0) {
                return false;
            }

            // sort list by distance
            checkTargetSeenList.Sort((x, y) => { return (x.distance).CompareTo((y.distance)); });
            
            if (blaze.enemyToAttack.transform.IsChildOf(checkTargetSeenList[0].transform) || checkTargetSeenList[0].transform.IsChildOf(blaze.enemyToAttack.transform)) {
                return true;
            }
