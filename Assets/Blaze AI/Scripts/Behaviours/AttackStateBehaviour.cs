    ...
    public class AttackStateBehaviour : MonoBehaviour
    {
        ...
        public virtual void Update()
        {
            ...
            if (!calculatedLastEnemyPos) {
                ...
                lastEnemyPos = blaze.RandomizePosition(blaze.enemyColPoint, blaze.navmeshAgent.radius + 1.5f);
                
                if (!blaze.IsPathReachable(lastEnemyPos)) {
                    lastEnemyPos = blaze.ValidateYPoint(blaze.enemyColPoint);
                }
            }
        ...
        {
            if (attacks.Length <= 0) {
                blaze.PrintWarning(blaze.warnAnomaly, "Attack operation skipped because attack animations list is empty in Attack State Behaviour. Please add an item to the list. You can even leave the animation empty but atleast set the duration otherwise the duration is unknown and can't attack.");
