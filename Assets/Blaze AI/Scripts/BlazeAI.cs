...
public class BlazeAI : MonoBehaviour 
{
    ...
    List<RaycastHit> checkObjVisibleRayHitList = new List<RaycastHit>();
    ...
    public virtual bool MoveTo(Vector3 location, float moveSpeed, float turnSpeed, string animName=null, float animT=0.25f, string dir="front", float closestPointDistance=0) 
    {
        ...
        if (dir == "front") 
        {
            ...
            if ((!isAttacking || enemyToAttack == null) && (lastCalculatedPath == location) && cornersQueue.Count == 0) {
                ...
                if (dist <= (minDis * minDis) || location == transform.position || pathCorner == Vector3.zero) {
    ...
    public virtual void VisionCheck()
    {
        ...
        if (previousEnemy != null && previousEnemy != enemiesToAttack[0].transform) {
            ResetEnemyManager();
        }

        if (vision.useVisionMeter && enemyToAttack == null) {
            if (!CheckVisionMeter(true, enemiesToAttack[0].transform.gameObject, range)) return;
        }
    ...
    public bool CheckObjectCompletelyVisible(GameObject go, int layersToHit, int minDetectionScore)
    {
        ...
        checkObjVisibleRayHitList.Clear();
        ...
        for (int i=0; i<hits; i++) 
        {
            ...
            checkObjVisibleRayHitList.Add(checkObjVisibleRayHitArr[i]);
        }

        if (checkObjVisibleRayHitList.Count > 0) {
            checkObjVisibleRayHitList.Sort((x, y) => { return (x.distance).CompareTo((y.distance)); });
            
            if (go.transform.IsChildOf(checkObjVisibleRayHitList[0].transform) || checkObjVisibleRayHitList[0].transform.IsChildOf(go.transform)) {
                detectionScore++;
            }
        }
